...
(function(e) {
  ...
  e.fn.vectorMap = function(e2) {
    var n, r, i, n = this.children(".jvectormap-container").data("mapObject");
    if (e2 === "addMap")
      jvm.WorldMap.maps[arguments[1]] = arguments[2];
    else {
      if (!(e2 !== "set" && e2 !== "get" || !t[e2][arguments[1]]))
        return r = arguments[1].charAt(0).toUpperCase() + arguments[1].substr(1), n[e2 + r].apply(n, Array.prototype.slice.call(arguments, 2));
      e2 = e2 || {}, e2.container = this, n = new jvm.WorldMap(e2);
    }
    return this;
  };
})(jQuery), function(e) {
  function r(t2) {
    var n2 = t2 || window.event, r2 = [].slice.call(arguments, 1), i = 0, s = true, o = 0, u = 0;
    return t2 = e.event.fix(n2), t2.type = "mousewheel", n2.wheelDelta && (i = n2.wheelDelta / 120), n2.detail && (i = -n2.detail / 3), u = i, n2.axis !== void 0 && n2.axis === n2.HORIZONTAL_AXIS && (u = 0, o = -1 * i), n2.wheelDeltaY !== void 0 && (u = n2.wheelDeltaY / 120), n2.wheelDeltaX !== void 0 && (o = -1 * n2.wheelDeltaX / 120), r2.unshift(t2, i, o, u), (e.event.dispatch || e.event.handle).apply(this, r2);
  }
  ...
  e.event.special.mousewheel = { setup: function() {
    if (this.addEventListener)
      for (var e2 = t.length; e2; )
        this.addEventListener(t[--e2], r, false);
    else
      this.onmousewheel = r;
  }, teardown: function() {
    if (this.removeEventListener)
      for (var e2 = t.length; e2; )
        this.removeEventListener(t[--e2], r, false);
    else
...
jvm.$ = jQuery, jvm.AbstractElement = function(e, t) {
  ...
  this.style = n || {}, this.style.current = {}, this.isHovered = false, this.isSelected = false, this.updateStyle();
  ...
  try {
    ...
    jvm.VMLElement.prototype.createElement = function(e2) {
      return document.createElement("<" + e2 + ' xmlns="urn:schemas-microsoft.com:vml" class="rvml">');
    };
  }
  ...
  jvm.VMLShapeElement.parentClass.call(this, e, t), this.fillElement = new jvm.VMLElement("fill"), this.strokeElement = new jvm.VMLElement("stroke"), this.node.appendChild(this.fillElement.node), this.node.appendChild(this.strokeElement.node), this.node.stroked = false, jvm.AbstractShapeElement.apply(this, arguments);
  ...
  switch (e) {
    ...
    case "fill-opacity":
      ...
      break;
    case "stroke":
      t === "none" ? this.node.stroked = false : this.node.stroked = true, this.node.strokecolor = t;
      break;
    case "stroke-opacity":
      this.strokeElement.node.opacity = Math.round(t * 100) + "%";
      break;
    case "stroke-width":
      parseInt(t, 10) === 0 ? this.node.stroked = false : this.node.stroked = true, this.node.strokeweight = t;
      break;
    case "d":
  ...
  jvm.VMLPathElement.parentClass.call(this, "shape", e, t), this.node.coordorigin = "0 0", n.node.on = true, n.node.matrix = "0.01,0,0,0.01,0,0", n.node.offset = "0,0", this.node.appendChild(n.node);
  ...
  var t = "", n = 0, r = 0, i, s;
  return e = e.replace(/(-?\d+)e(-?\d+)/g, "0"), e.replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function(e2, t2, o, u) {
  ...
  e === "polynomial" ? this.normalize = function(e2) {
    return Math.pow(e2, 0.2);
  ...
  var t = this, n;
  this.params = jvm.$.extend(true, {}, jvm.WorldMap.defaultParams, e);
  ...
  this.container.mousemove(function(i) {
    return e && (r.transX -= (t - i.pageX) / r.scale, r.transY -= (n - i.pageY) / r.scale, r.applyTransform(), t = i.pageX, n = i.pageY), false;
  }).mousedown(function(r2) {
    return e = true, t = r2.pageX, n = r2.pageY, false;
  }), jvm.$("body").mouseup(function() {
    e = false;
  }), this.params.zoomOnScroll && this.container.mousewheel(function(e2, t2, n2, i) {
    var s = jvm.$(r.container).offset(), o = e2.pageX - s.left, u = e2.pageY - s.top, a = Math.pow(1.3, i);
    r.label.hide(), r.setScale(r.scale * a, o, u), e2.preventDefault();
  });
}, bindContainerTouchEvents: function() {
  var e, t, n = this, r, i, s, o, u, a = function(a2) {
    var f = a2.originalEvent.touches, l, c, h, p;
    a2.type == "touchstart" && (u = 0), f.length == 1 ? (u == 1 && (h = n.transX, p = n.transY, n.transX -= (r - f[0].pageX) / n.scale, n.transY -= (i - f[0].pageY) / n.scale, n.applyTransform(), n.label.hide(), (h != n.transX || p != n.transY) && a2.preventDefault()), r = f[0].pageX, i = f[0].pageY) : f.length == 2 && (u == 2 ? (c = Math.sqrt(Math.pow(f[0].pageX - f[1].pageX, 2) + Math.pow(f[0].pageY - f[1].pageY, 2)) / t, n.setScale(e * c, s, o), n.label.hide(), a2.preventDefault()) : (l = jvm.$(n.container).offset(), f[0].pageX > f[1].pageX ? s = f[1].pageX + (f[0].pageX - f[1].pageX) / 2 : s = f[0].pageX + (f[1].pageX - f[0].pageX) / 2, f[0].pageY > f[1].pageY ? o = f[1].pageY + (f[0].pageY - f[1].pageY) / 2 : o = f[0].pageY + (f[1].pageY - f[0].pageY) / 2, s -= l.left, o -= l.top, e = n.scale, t = Math.sqrt(Math.pow(f[0].pageX - f[1].pageX, 2) + Math.pow(f[0].pageY - f[1].pageY, 2)))), u = f.length;
  };
  ...
  this.container.mousemove(function() {
    t = true;
  }), this.container.delegate("[class~='jvectormap-element']", "mouseover mouseout", function(t2) {
    ...
    t2.type == "mouseover" ? (e.container.trigger(f, [s]), f.isDefaultPrevented() || o.setHovered(true), e.label.text(u), e.container.trigger(a, [e.label, s]), a.isDefaultPrevented() || (e.label.show(), e.labelWidth = e.label.width(), e.labelHeight = e.label.height())) : (o.setHovered(false), e.label.hide(), e.container.trigger(i + "Out.jvectormap", [s]));
  }), this.container.delegate("[class~='jvectormap-element']", "mousedown", function(e2) {
    t = false;
  ...
  if (jvm.$.isArray(e) || this.regions[e]) {
    ...
    this.setScale(Math.min(this.width / r.width, this.height / r.height), -(r.x + r.width / 2), -(r.y + r.height / 2), true);
  } else
    e *= this.baseScale, this.setScale(e, -t * this.defaultWidth, -n * this.defaultHeight, true);
  ...
  for (e in this.mapData.paths)
    t = this.canvas.addPath({ d: this.mapData.paths[e].path, "data-code": e }, jvm.$.extend(true, {}, this.params.regionStyle)), jvm.$(t.node).bind("selected", function(e2, t2) {
      n.container.trigger("regionSelected.jvectormap", [jvm.$(this).attr("data-code"), t2, n.getSelectedRegions()]);
  ...
  for (t in e)
    i = e[t] instanceof Array ? { latLng: e[t] } : e[t], r = this.getMarkerPosition(i), r !== false && (n = this.canvas.addCircle({ "data-index": t, cx: r.x, cy: r.y }, jvm.$.extend(true, {}, this.params.markerStyle, { initial: i.style || {} }), this.markersGroup), n.addClass("jvectormap-marker jvectormap-element"), jvm.$(n.node).bind("selected", function(e2, t2) {
      o.container.trigger("markerSelected.jvectormap", [jvm.$(this).attr("data-index"), t2, o.getSelectedMarkers()]);
    }), this.markers[t] && this.removeMarkers([t]), this.markers[t] = { element: n, config: i });
}, repositionMarkers: function() {
  var e, t;
  for (e in this.markers)
    t = this.getMarkerPosition(this.markers[e].config), t !== false && this.markers[e].element.setStyle({ cx: t.x, cy: t.y });
  ...
  return t < -180 + i && (t += 360), n = jvm.Proj[r.type](e, t, i), u = this.getInsetForPoint(n.x, n.y), u ? (a = u.bbox, n.x = (n.x - a[0].x) / (a[1].x - a[0].x) * u.width * this.scale, n.y = (n.y - a[0].y) / (a[1].y - a[0].y) * u.height * this.scale, { x: n.x + this.transX * this.scale + u.left * this.scale, y: n.y + this.transY * this.scale + u.top * this.scale }) : false;
