...
!function($) {
  "use strict";
  $.event.special.destroyed || ($.event.special.destroyed = { remove: function(o) {
    o.handler && o.handler();
    ...
    function place(currentInput, maxLengthIndicator) {
      var pos = function(currentInput2) {
        var el = currentInput2[0];
        return $.extend({}, "function" == typeof el.getBoundingClientRect ? el.getBoundingClientRect() : { width: el.offsetWidth, height: el.offsetHeight }, currentInput2.offset());
      }(currentInput);
      if ("function" !== $.type(options.placement))
        if ($.isPlainObject(options.placement))
          !function(placement, maxLengthIndicator2) {
            if (placement && maxLengthIndicator2) {
              var cssPos = {};
              $.each(["top", "bottom", "left", "right", "position"], function(i, key) {
                var val = options.placement[key];
                void 0 !== val && (cssPos[key] = val);
              }), maxLengthIndicator2.css(cssPos);
            }
    ...
    return $.isFunction(options) && !callback && (callback = options, options = {}), options = $.extend({ showOnReady: false, alwaysShow: true, threshold: 0, warningClass: "small form-text text-muted", limitReachedClass: "small form-text text-danger", limitExceededClass: "", separator: " / ", preText: "", postText: "", showMaxLength: true, placement: "bottom-right-inside", message: null, showCharsTyped: true, validate: false, utf8: false, appendToParent: false, twoCharLinebreak: true, customMaxAttribute: null, customMaxClass: "overmax", allowOverMax: false, zIndex: 1099 }, options), this.each(function() {
      var maxLengthCurrentInput, maxLengthIndicator, currentInput = $(this);
      function firstInit() {
        var maxlengthContent = updateMaxLengthHTML(currentInput.val(), maxLengthCurrentInput, "0");
        maxLengthCurrentInput = getMaxLength(currentInput), maxLengthIndicator || (maxLengthIndicator = $('<span class="bootstrap-maxlength"></span>').css({ display: "none", position: "absolute", whiteSpace: "nowrap", zIndex: options.zIndex }).html(maxlengthContent)), currentInput.is("textarea") && (currentInput.data("maxlenghtsizex", currentInput.outerWidth()), currentInput.data("maxlenghtsizey", currentInput.outerHeight()), currentInput.mouseup(function() {
      ...
      $(window).resize(function() {
        ...
        return options.validate && remaining < 0 ? (!function(input, maxlength2) {
          var text = input.val();
          if (options.twoCharLinebreak && "\n" === (text = text.replace(/\r(?!\n)|\n(?!\r)/g, "\r\n"))[text.length - 1] && (maxlength2 -= text.length % 2), options.utf8) {
            for (var indexedSize = text.split("").map(utf8CharByteCount), removedBytes = 0, bytesPastMax = utf8Length(text) - maxlength2; removedBytes < bytesPastMax; removedBytes += indexedSize.pop())
              ;
            maxlength2 -= maxlength2 - indexedSize.length;
          }
          input.val(text.substr(0, maxlength2));
        }(currentInput, maxlength), output = false) : manageRemainingVisibility(remaining, currentInput, maxLengthCurrentInput, maxLengthIndicator), output;
      });
    });
  } });
}(jQuery);
//# sourceMappingURL=bootstrap-maxlength_dist_bootstrap-maxlength__min__js.js.map
